...
BASE_DIR = ''
ORIGIN_INPUTS_DIR = os.path.join(BASE_DIR, 'artificial_images')
ORIGIN_TEACHERS_DIR = os.path.join(BASE_DIR, 'annot')
...
def image_data_augmentaiton(file_name, input_img, teacher_img
    ...
    for name, inp_img, tea_img in data:
        ...
        input_save_path = os.path.join(INPUTS_DIR, name + '.jpg')
...
def scaling_image(img, scale):
    ...
    img_resize = cv2.resize(img, (res_w, res_h))
    if scale < 1:
